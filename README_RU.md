...
2. Пройдите аутентификацию через конечную точку `/auth/token`, чтобы получить токен JWT
3. Используйте токен для доступа к защищенным конечным точкам
4. Создавайте задачи через конечную точку `/tasks`
5. Отслеживайте статус задач через конечную точку `/tasks/{task_id}`
6. Администраторы могут использовать конечные точки `/admin/*` для управления системой
7. Доступ к админ-панели по адресу `http://localhost:3000` для графического интерфейса
